...
public static $cache = null;
    ...
    public static function activate()
    {
        ...
        \Typecho\Plugin::factory('Widget_Archive')->footer = [__CLASS__, 'seo_autopush'];
          }
        //腾讯云COS
    ...
    public static function C()
    {
        ...
        try {
            ...
            if (!empty($data)) {
               ...
               $html = str_replace(gethostbyname($_SERVER['HTTP_HOST']),$op->siteUrl,$data['html']);
...
/**
 ...
 public static function secretComment($data, $obj,$last){
     ...
     $stext = '<div class="ui left pointing basic label break bshideContent" style="margin-left:auto">该评论为私密评论，仅已登录者与评论双方可见</div>';
     $text = '<div class="ui left pointing basic label break" style="margin-left:auto">'.str_replace("@私密@","",$text).'</div>';
 ...
 if (\Typecho\Widget::widget('Widget_User')->hasLogin()){
   if($user->group=="administrator"||$user->group=="editor"||$user->uid==$obj->authorId||$user->uid==$obj->ownerId||in_array($user->uid,$parentid)){
       Helper::options()->commentsHTMLTagAllowed='<div class="ui left pointing basic label break" style="margin-left:auto">'.Helper::options()->commentsHTMLTagAllowed;
     return $text;
   }else{
     Helper::options()->commentsHTMLTagAllowed='<div class="ui left pointing basic label break bshideContent" style="margin-left:auto">'.Helper::options()->commentsHTMLTagAllowed;
     return $stext;
   }
}else{
...
if(($obj->mail==$commentsMail && $obj->authorId==0)||(!empty($parentmail) && in_array($commentsMail,$parentmail))){
 ...
 if(Plugin::xsscheck($comment['text'])) {
        ...
        $comment['mail'] = htmlspecialchars($comment['mail'],ENT_QUOTES,'UTF-8');
  if($comment['url'] && Plugin::xsscheck($comment['url'])) {
			\Typecho\Cookie::set('__typecho_remember_text', $comment['text']);
			throw new \Typecho\Widget\Exception("抱歉，系统检测到您的评论包含危险内容，请返回修改后再试。",200);
        }
        if($comment['mail'] && Plugin::xsscheck($comment['mail'])) {
			\Typecho\Cookie::set('__typecho_remember_text', $comment['text']);
			throw new \Typecho\Widget\Exception("抱歉，系统检测到您的评论包含危险内容，请返回修改后再试。",200);
        }
 ...
 //代码高亮
    ...
    if(bsOptions::getInstance()::get_option('bearsimple' )['Codehightlight'] == true){
       ...
       $style = bsOptions::getInstance()::get_option( 'bearsimple' )['code_style'];
        ...
        $cssUrl = $dir.'modules/codehightlight/static/styles/' . $style.'?v=2.2.1';
...
HTML;
 ...
 self::$_isMailLog = in_array('4', $Temoptions['CommentNotify__OtherSetting']) ? true : false;
    ...
    {
        $api = get_option('bearsimple')['seotabs']['baidu_token'];
        $siteUrl = Helper::options()->adminUrl;
        if($content['created'] >  Helper::options()->time) exit('<script>location.href="'.$siteUrl.'/manage-posts.php";</script>');
        if($api === 'NULL' || strpos($api, 'data.zz.baidu.com') !== 7) exit('<script>alert("请配置正确的百度收录推送接口调用地址");location.href="'.$siteUrl.'/manage-posts.php";</script>');
        ...
        if(isset($res['error'])) exit('<script>alert("链接提交百度过程中出现错误，错误代码：'.$res['error'].'，错误信息：'.$res['message'].'。");location.href="'.$siteUrl.'/manage-posts.php";</script>');
    }
